...
Public Class miniTool
    ...
    Private Sub miniTool_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub loadtills()
        Dim con As New SqlConnection("Data Source=" & ComboBox1.Tag & ";Database=TPCentralDB;" & Main.cred & ";")
    ...
    Public Sub moveserverlog(k)
        ...
        If k = 1 Then
            ...
            s = i1()
        Else
            str = Main.serverlist.SelectedItems(0).Group.ToString & "_" & Main.serverlist.SelectedItems.Item(0).SubItems(0).Text & "_" & Main.serverlist.SelectedItems.Item(0).SubItems(1).Text & "_IP_" & Main.serverlist.SelectedItems.Item(0).SubItems(2).Text
            loc = "\\" & Main.serverlist.SelectedItems.Item(0).SubItems(2).Text & "\e$\TpDotnet\Log"
            s = vbCrLf & str
        End If
        ...
        Try
            If FolderBrowserDialog1.ShowDialog() = Windows.Forms.DialogResult.OK Then
                If (Label2.Text = "ON" And k = 1) Or (Main.serverlist.SelectedItems.Item(0).SubItems(3).Text = "ON" And k = 0) Then
    ...
    Public Sub movetilllog(k)
        ...
        If k = 1 Then
            ...
            s = i2()
        Else
            str = Main.serverlist.SelectedItems(0).Group.ToString & "_" & Main.serverlist.SelectedItems.Item(0).SubItems(0).Text & "_" & Main.tilllist.SelectedItems.Item(0).SubItems(2).Text & "_" & Main.tilllist.SelectedItems.Item(0).SubItems(1).Text & "_" & Main.tilllist.SelectedItems.Item(0).SubItems(3).Text & "_IP_" & Main.tilllist.SelectedItems.Item(0).SubItems(5).Text
            loc = "\\" & Main.tilllist.SelectedItems.Item(0).SubItems(5).Text & "\e$\TpDotnet\Log"
            s = vbCrLf & str
        End If
        ...
        Try
            If FolderBrowserDialog1.ShowDialog() = Windows.Forms.DialogResult.OK Then
                If (Label3.Text = "ON" And k = 1) Or (Main.tilllist.SelectedItems.Item(0).SubItems(6).Text = "ON" And k = 0) Then
                    If My.Computer.FileSystem.DirectoryExists(loc) Then
                        Process.Start("C:\Program Files\7-Zip\7z.exe", "a " & quote & FolderBrowserDialog1.SelectedPath & fname & quote & " " & loc)
